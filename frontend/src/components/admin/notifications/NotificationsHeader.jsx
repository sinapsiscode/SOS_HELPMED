...
/**
 * Header del sistema de notificaciones con tabs
...
const NotificationsHeader = ({ activeTab, activeAlerts, onTabChange }) => {
  ...
  if (typeof activeTab !== 'string') {
    console.error('NotificationsHeader: activeTab debe ser un string')
    return null
  }

  if (!Array.isArray(activeAlerts)) {
    console.error('NotificationsHeader: activeAlerts debe ser un array')
    return null
  }

  if (typeof onTabChange !== 'function') {
    console.error('NotificationsHeader: onTabChange debe ser una función')
    return null
  }
  ...
  return (
    <div className="bg-white rounded-xl shadow-medium p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 space-y-2 sm:space-y-0">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Alertas y Notificaciones</h1>
