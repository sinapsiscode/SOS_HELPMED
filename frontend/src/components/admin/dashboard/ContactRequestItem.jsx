...
const ContactRequestItem = ({ request, onContactClient, onViewDetail }) => {
  const labels = LABELS.admin.dashboard.contactRequestItem
  ...
  return (
    <div className="bg-white border-b border-gray-200 px-4 py-3">
      ...
      <div className="flex gap-2 mb-3 ml-9">
        ...
        {request.serviceType === labels.serviceTypes.programmedTransfer && (
      ...
      <div className="mb-3 ml-9">
        <div className="text-sm text-gray-700 mb-1">
          <span className="font-medium">{labels.fields.reason}:</span> {labels.fields.protectedZoneMessage}
        </div>
        <div className="text-sm text-gray-700">
          <span className="font-medium">{labels.fields.plan}:</span> {request.planName}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3 ml-9">
        ...
        <div className="bg-blue-50 rounded-lg p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-blue-700">{labels.cards.serviceUsage.title}</span>
          </div>
          <div className="text-sm font-semibold text-blue-800">
            {labels.cards.serviceUsage.content}
          </div>
        </div>
        ...
        <div className="bg-green-50 rounded-lg p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-green-700">{labels.cards.contact.title}</span>
          </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
        <div className="text-xs text-gray-500 ml-9">
          <span>{labels.dates.request}: {formatDate(request.requestDate)}</span>
          <span className="ml-4">{labels.dates.lastService}: {formatDate(request.lastServiceDate)}</span>
        </div>
